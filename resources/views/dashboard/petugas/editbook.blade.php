        ...
        <div class="p-4 sm:ml-64 md:mt-20">
            <div class="p-4 border-2 border-gray-200 border-dashed rounded-lg dark:border-gray-700 h-auto">
                ...
                {{-- @if ($errors->any())
                    ...
                    @endforeach
                @endif --}}
                <form class="mx-auto max-w-5xl flex flex-col justify-start text-white" action="{{ route('book.update', ['slug' => $book->slug]) }}"
                ...
                @if (session('error'))
                    {{  session('error')  }}
                @endif
                    <img id="imagePreview" src="{{ $book->image != null ? asset('storage/'.$book->image) : asset('img/logo.png') }}" alt="Preview" 
                    ...
                    <div class="mb-5 mt-5 font-Mont font-bold">
                        ...
                        <input type="text" id="name" name="title" value="{{ $book->title }}"
                    ...
                    <div class="mb-5 font-Mont font-bold">
                        ...
                        <input type="text" id="email" name="body" value="{{ $book->body }}"
                            class="shadow-sm bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500 dark:shadow-sm-light"
                            required>
                    </div>
                    <div class="mb-3 font-Mont font-bold">
                        ...
                        <select id="countries" name="category_id"
                            ...
                            name="role_id">
                            <option selected value="{{ $book->category_id }}">Select Category</option>

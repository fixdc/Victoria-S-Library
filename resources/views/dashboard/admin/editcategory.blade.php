        ...
        <div class="p-4 sm:ml-64 md:mt-20 h-full">
            <div class="p-4 border-2 border-gray-200 border-dashed rounded-lg dark:border-gray-700 h-screen">
                ...
                @if ($errors->any())
                    ...
                    @endforeach
                @endif
                <form class="mx-auto max-w-5xl flex flex-col justify-start text-white" action="/dashboard/admin/editcategory/{{$category->category_name}}" method="POST" >
                    @csrf
                    @method('put')
                    <div class="mb-5 font-Mont font-bold">
                        <label for="email" class="block font-bold mb-2 text-sm text-white">Name</label>
                        <input type="text" id="name" name="category_name" value="{{ $category->category_name }}"
